...
for seed in range(1, 31):
    ...
    predict_KNN = KNN_model.predict(x_teste_norm)
    accuracy_predict_KNN.append(accuracy_score(y_teste, predict_KNN))
    
    SVC_model.fit(x_treino_norm, y_treino)
    predict_SVC = SVC_model.predict(x_teste_norm)
    accuracy_predict_SVC.append(accuracy_score(y_teste, predict_SVC))
    
    MLP_Model.fit(x_treino_norm, y_treino)
    predict_MLP = MLP_Model.predict(x_teste_norm)
...
print("--------------------------------------------------------------")
print ("A acurácia da predição do KNN foi de {:.2f}%.".format(mean(accuracy_predict_KNN)*100))
print ("A acurácia da predição do SVM foi de {:.2f}%.".format(mean(accuracy_predict_SVC)*100))
print ("A acurácia da predição do MLP foi de {:.2f}%.".format(mean(accuracy_predict_MLP)*100))
